<div class="bg-dark text-center text-lg-start">
	<!-- Grid container -->
	<div class="container p-4">
	<!--Grid row-->
	<div class="row">
		<!--Grid column-->
		<div class="col-lg-6 col-md-12 mb-4 mb-md-0">
		<h5 class="text-white">The Kickbackers</h5>

		<p class="text-white">The Kickbackers was founded in 2021</p>
		</div>
		<!--Grid column-->

		<!--Grid column-->
		<div class="col-lg-3 col-md-6 mb-4 mb-md-0">
		<h5 class="text-white">Get Around</h5>

		<ul class="list-unstyled mb-0">
			<li>
			<a href="aboutus.php" class="text-white">About Us</a>
			</li>
			<li>
			<a href="productlisting.php" class="text-white">Product</a>
			</li>
			<li>
			<a href="#!" class="text-white">Contact Us</a>
			</li>
			<li>
			<a href="#!" class="text-white">Gallery</a>
			</li>
		</ul>
		</div>
		<!--Grid column-->

		<!--Grid column-->
		<div class="col-lg-3 col-md-6 mb-4 mb-md-0">
		<h5 class="text-white">We Accept</h5>
		<ul class="list-unstyled mb-0">
			<li>
			<a class="visa"><img src="image/Visa.png" alt="Visa" width="6" height="5"></a>
			</li>
			<li>
			<a class="master"><img src="image/mastercard.png" alt="mastercard" width="6" height="2"></a>
			</li>
			<li>
			<a class="paypal"><img src="image/paypal.png" alt="paypal" width="6" height="2"></a>
			</li>
			<li>
			<a class="american_express"><img src="image/American_Express.png" alt="American_Express" width="6" height="4"></a>
			</li>
		</ul>
		</div>

		<!--Grid column-->
	</div>
	<!--Grid row-->
		
	</div>
	<!-- Grid container -->

	<!-- Copyright -->
	<div class="text-white" style="background-color: rgba(0, 0, 0, 0.2);">
	© 2020 Copyright:
	<a class="text-white text-center" href="https://mdbootstrap.com/">MDBootstrap.com</a>
	</div>
</div>
<!-- Copyright -->
